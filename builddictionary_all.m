function [ Signature, TypeNum, Sc, Tc ] = builddictionary_all( ...
...
%%
% roots{2} = zscore(roots{2});
% for type = 1:2:3
%     Endmember = zscore(roots{type});
%    
%         centers = 40;
%         while(centers<=44)
%             flag = 0;
%             while(flag == 0)
%                 [IDX, C] = kmeans(Endmember', centers, ...
%                      'EmptyAction', 'drop');
%                 if ~isnan(sum(C))
%                     flag = 1;
%                 end
...
%             flag = 1;
% %             for k=1:centers
% %                 error = sum(sum(abs(Endmember(:,IDX==k)'-repmat(C(k,:),sum(IDX==k),1))))/sum(IDX==k);
% %                 if error > 100
% %                     flag = 0;
% %                     centers = centers + 1;
% %                     break
...
for i=1:nc
    temp = Endmember(:,i);
    % shift untile 13s is left
    sig = [];
    for seg = 1:301:stream_length
        vectemp = temp(seg:seg+300);
        segsig = [];
        for j=1:step:(301-13/0.1)
            sigtemp = vectemp;
            sigtemp(1:j) = sigtemp(1);
            sigtemp(j:end) = vectemp(1:end-j+1);
            segsig = [segsig , sigtemp];
        end
        sig = [sig ; segsig];
    end             

% nn_L1QP_FeatureSign solves nonnegative quadradic programming 
% using Feature Sign. 
%
%    min  0.5*x'*A*x+b'*x+\lambda*|x|, s.t. x>0
%

function [ x ] = sparsecoding( lambda, A, b )
...
%         o_new=(vect(idx)/2 + ba(idx))'*x_new(idx) + lambda*sum((x_new(idx)));
...
%             o_s = (Aa(idx, idx)*x_s(idx)/2 + ba(idx))'*x_s(idx)+lambda*sum((x_s(idx)));
            if o_s<o_min,
                x_min=x_s;
                o_min=o_s;
            end
